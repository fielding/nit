...
check "branch create passthrough" \
  "git branch git-created HEAD~1 >/dev/null && git branch --list git-created | sed 's/git-created/created/'" \
  "$NIT branch nit-created HEAD~1 >/dev/null && git branch --list nit-created | sed 's/nit-created/created/'"
...
check "show --stat totals consistent" \
  "$NIT show --stat $FIRST_HASH 2>&1 | tail -n +3 | grep -o '+[0-9]*' | tr -d '+' | awk '{s += \$1} END {print s}'" \
...
check "show HEAD:binary.dat matches git" \
  "git show HEAD:binary.dat | od -An -tx1" \
  "$NIT show HEAD:binary.dat 2>&1 | od -An -tx1"

# --- NIT_COLORS env var ---
echo ""
echo "NIT_COLORS:"

# Force a pseudo-TTY for color assertions. util-linux script uses -c;
# BSD/macOS script accepts the command after the typescript path.
if command -v script >/dev/null 2>&1; then
  SCRIPT_USES_C_FLAG=0
  if script -q -c "printf test" /dev/null >/dev/null 2>&1; then
    SCRIPT_USES_C_FLAG=1
  fi

  script_tty() {
    if [ "$SCRIPT_USES_C_FLAG" = 1 ]; then
      local cmd=""
      local arg
      for arg in "$@"; do
  ...
  default_out=$(script_tty "$NIT" show -H | cat -v | head -3)
  custom_out=$(script_tty env NIT_COLORS="hash=35" "$NIT" show -H | cat -v | head -3)
  ...
  truecolor_out=$(script_tty env NIT_COLORS="hash=38;2;255;0;0" "$NIT" show -H | cat -v | head -1)
  ...
  dup_out1=$(script_tty env NIT_COLORS="hash=35" "$NIT" show -H | cat -v | head -1)
  dup_out2=$(script_tty env NIT_COLORS="hash=33:hash=35" "$NIT" show -H | cat -v | head -1)
  ...
  custom_show=$(script_tty env NIT_COLORS="hash=35" "$NIT" show -H | cat -v | head -1)
  ...
  default_diff=$(script_tty "$NIT" diff -H | cat -v | head -3)
  custom_diff=$(script_tty env NIT_COLORS="add=35:del=36" "$NIT" diff -H | cat -v | head -3)
  ...
  default_log=$(script_tty "$NIT" log -H -n 1 | cat -v | head -1)
  custom_log=$(script_tty env NIT_COLORS="hash=35:date=36" "$NIT" log -H -n 1 | cat -v | head -1)
  ...
  default_status=$(script_tty "$NIT" status -H | cat -v)
  custom_status=$(script_tty env NIT_COLORS="staged=35:unstaged=36" "$NIT" status -H | cat -v)
